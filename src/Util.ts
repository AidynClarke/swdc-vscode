...
export function isActiveIntegration(type: string, integration: any) {
	if (integration && integration.status.toLowerCase() === "active") {
		...
		if (integration.integration_type) {
			return !!(
				integration.integration_type.type.toLowerCase() === type.toLowerCase()
			);
		}
...
export function getPluginUuid() {
	...
	if (!plugin_uuid) {
		...
		const hashName = require("node:crypto")
			.createHash("sha1")
			.update(name)
			.digest("hex");
...
export function getAuthCallbackState(autoCreate = true) {
	let auth_callback_state = getJsonItem(
		getDeviceFile(),
		"auth_callback_state",
		false,
	);
...
export function getSoftwareDir(name: string = SOFTWARE_DIRECTORY) {
	const homedir = os.homedir();
	const softwareDataDir = isWindows()
		? `${homedir}\\${name}`
		: process.env.XDG_CONFIG_HOME
			? `${process.env.XDG_CONFIG_HOME}/${name.substring(1)}`
			: `${homedir}/${name}`;
...
export function logIt(message: string, isError = false) {
	const windowMsg: string = isPrimaryWindow() ? "(p)" : "";
	outputChannel.appendLine(
		`${formatISO(new Date())} ${getLogId()}${windowMsg}: ${message}`,
	);
	if (isError) {
...
// biome-ignore lint/suspicious/noExplicitAny: <explanation>
export function humanizeMinutes(_min: any) {
	const min = Number.parseInt(_min, 0) || 0;
	let str = "";
	if (min === 60) {
		str = "1h";
	} else if (min > 60) {
		...
		if ((Number.parseFloat(_min) / 60) % 1 === 0) {
			str = hoursStr;
		} else {
