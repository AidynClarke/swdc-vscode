import { isGitProject, logIt, noSpacesProjectDir } from "../Util";
...
export function accumulateNumStatChanges(results: any): DiffNumStats[] {
  ...
  */
	...
	for (const result of results) {
		...
		if (
			Number.isInteger(diffNumStat.insertions) &&
			Number.isInteger(diffNumStat.deletions)
		)
...
export async function getDefaultBranchFromRemoteBranch(
	projectDir: string,
	remoteBranch: string,
): Promise<string> {
	...
	const remoteName = remotes
		...
		// biome-ignore lint/suspicious/noExplicitAny: <explanation>
		.find((r: any) => remoteBranch.includes(r));

	if (remoteName) {
		// Check if the remote has a HEAD symbolic-ref defined
		const headBranchList = execCmd(
		...
		if (headBranchList.length) {
			...
			const verify = execCmd(
				`git show-ref --verify '${headBranchList[0]}'`,
				projectDir,
				true,
			);
		...
		if (!defaultBranchFromRemoteBranch) {
			const assumedDefaultBranch = await guessDefaultBranchForRemote(
				projectDir,
				remoteName,
			);
	...
	if (!defaultBranchFromRemoteBranch) {
		...
		const remoteBranchesResult = execCmd(
			"git branch -r -l '*/HEAD'",
			projectDir,
			true,
		);
	...
	if (!defaultBranchFromRemoteBranch) {
		logIt(`remotes: ${JSON.stringify(remotes)}`);
		...
		for (const remote of remotes) {
			const assumedRemoteDefaultBranch = await guessDefaultBranchForRemote(
				projectDir,
				remote,
			);
...
async function guessDefaultBranchForRemote(
	projectDir: string,
	remoteName: string,
): Promise<string | undefined> {
	// Get list of branches for the remote
	const remoteBranchesList =
		execCmd(`git branch -r -l '${remoteName}/*'`, projectDir, true) || [];
	const possibleDefaultBranchNames = ["main", "master"];
	// biome-ignore lint/suspicious/noExplicitAny: <explanation>
	let assumedDefault: any;

	for (const possibleDefault of possibleDefaultBranchNames) {
		assumedDefault = remoteBranchesList.find(
			// biome-ignore lint/suspicious/noExplicitAny: <explanation>
			(b: any) => b.trim() === `${remoteName}/${possibleDefault}`,
		);
...
export async function getLatestCommitForBranch(
	projectDir: string,
	branch: string,
): Promise<string> {
...
export async function commitAlreadyOnRemote(
	projectDir: string,
	commit: string,
): Promise<boolean> {
	const resultList = execCmd(
	...
	authors = configUsers?.length
		? // biome-ignore lint/suspicious/noExplicitAny: <explanation>
			configUsers.map((configUser: any) => {
				const [_, ...author] = configUser.split(" ");
				return author.join(" ");
			})
		: [];
...
export async function getCommitsForAuthors(
	projectDir: string,
	branch: string,
	startRef: string,
	authors: string[],
	...
	const range =
		startRef !== "" ? `${startRef}..HEAD` : `HEAD --since="2 weeks ago"`;
...
export async function getChangesForCommit(
	projectDir: string,
	commit: string,
): Promise<DiffNumStats[]> {
...
function normalizeRepoIdentifier(_identifier: string) {
 ...
 */
export function getRepoIdentifierInfo(_identifier: string) {
	const identifier = normalizeRepoIdentifier(_identifier);
