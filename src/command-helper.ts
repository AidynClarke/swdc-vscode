...
import type { KpmManager } from "./managers/KpmManager";
...
import { authLogin, createAnonymousUser } from "./menu/AccountManager";
...
export function createCommands(
	...
	cmds.push(
		commands.registerCommand(
			"codetime.updateSidebarSettings",
			// biome-ignore lint/suspicious/noExplicitAny: <explanation>
			(payload: any) => {
				progressIt("Updating settings...", updateSettings, [
					payload.path,
					payload.json,
					true,
				]);
	...
	cmds.push(
		commands.registerCommand("codetime.logout", async () => {
			...
			if (user?.registered) {
				...
				showInformationMessage(
					"Successfully logged out of your Code Time account",
				);
				await reload();
			}
		}),
	...
	cmds.push(
		authentication.onDidChangeSessions(async (e) => {
			await authentication.getSession(AUTH_TYPE, ["profile"], {
				createIfNone: false,
			});
		}),
	);
