...
export async function initializeStatusBar() {
	ctMetricStatusBarItem = window.createStatusBarItem(
	...
	ctFlowModeStatusBarItem = window.createStatusBarItem(
		StatusBarAlignment.Right,
		9,
	);
...
export async function updateFlowModeStatusBar() {
	...
	const prevCmd: any | undefined = ctFlowModeStatusBarItem
		? ctFlowModeStatusBarItem.command
		: undefined;
	const { flowModeCommand, flowModeText, flowModeTooltip } =
		await getFlowModeStatusBarInfo();
...
export function updateStatusBarWithSummaryData() {
	...
	const averageDailyMinutes = Number(
		getJsonItem(getSessionSummaryFile(), "averageDailyMinutes"),
	);
	const currentDayMinutes = Number(
		getJsonItem(getSessionSummaryFile(), "currentDayMinutes"),
	);
	const inFlowIcon =
...
function showStatus(msg: string, _tooltip: string | null) {
	let tooltip =
		_tooltip ?? "Code time today. Click to see more from Code Time.";
