...
export class ExtensionManager {
	...
	private INSTALLED_ACTION = "installed";
	private UNINSTALLED_ACTION = "uninstalled";
	...
	private reconcileInstalledAndUninstalledPlugins(): void {
		...
		const missingPlugins: any[] = Object.keys(extensionData)
			.map((key: string) => {
	...
	private getInstalledPlugins(now: string): any[] {
		...
		const extensionData: any = getJsonItem(extensionsFile, "data", {});
		const os = getOs();
		const plugins = extensions.all
			.filter(
				(extension: any) =>
					extension.packageJSON.publisher != "vscode" &&
