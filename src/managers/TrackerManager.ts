import type { DocChangeInfo, ProjectChangeInfo } from "@swdotcom/editor-flow";
...
import type { KpmItem } from "../model/models";
import {
	authors,
...
export class TrackerManager {
	...
	private pluginParams = this.getPluginParams();
	...
	public async trackCodeTimeEvent(projectChangeInfo: ProjectChangeInfo) {
		...
		for await (const file of fileKeys) {
			...
			const repoParams = await this.getRepoParams(
				projectChangeInfo.project_directory,
			);
			...
			logIt(`codetime_event: ${JSON.stringify(codetime_event, undefined, 2)}`);
			storeMetrics(codetime_event);
		}
	}
	...
	public async trackUIInteraction(item: KpmItem) {
		...
		storeUIInteraction(ui_event);
	}

	public async trackGitLocalEvent(
		gitEventName: string,
		branch?: string,
		commit?: string,
	) {
	...
	private async trackUncommittedChangeGitEvent(
		projectParams: ReturnType<typeof this.getProjectParams>,
	) {
		const uncommittedChanges = await this.getUncommittedChangesParams(
	...
	private async trackLocalCommitGitEvent(
		projectParams: ReturnType<typeof this.getProjectParams>,
		branch: string,
		_commit?: string,
	) {
		const commit =
			_commit ??
			(await getLatestCommitForBranch(projectParams.project_directory, branch));
		...
		const commitInfo = await getInfoForCommit(
			projectParams.project_directory,
			commit,
		);
		const file_changes = await getChangesForCommit(
			projectParams.project_directory,
			commit,
		);
		const eventData = {
			commit_id: commit,
			git_event_timestamp: commitInfo.authoredTimestamp,
			file_changes,
		};
	...
	private async trackBranchCommitGitEvent(
		// biome-ignore lint/suspicious/noExplicitAny: <explanation>
		projectParams: any,
		remoteBranch: string,
		event_path: string,
	) {
		const defaultBranch = await getDefaultBranchFromRemoteBranch(
			projectParams.project_directory,
			remoteBranch,
		);
		...
		for (const commit of commits) {
			...
			const eventData = {
				...
				git_event_timestamp: commit.authoredTimestamp,
				file_changes,
			};
	...
	public async trackEditorAction(entity: string, type: string, event?: any) {
		...
		storeEditorActionEvent(editor_event);
	}
	...
	public async trackVSCodeExtension(eventData: any) {
		...
		storeEditorActionEvent(vscode_extension_event);
	}
	...
	getJwtParams() {
		const token: string = getItem("jwt");
		if (token?.match(/\s/)) {
	...
	getProjectParams() {
		...
		const project_directory = workspaceFolders.length
			? workspaceFolders[0].uri.fsPath
			: "";
		const project_name = workspaceFolders.length
	...
	getFileParams(event: any, projectRootPath: string) {
		...
		return {
			...
			syntax:
				textDoc.languageId || textDoc.fileName?.split(".")?.slice(-1)?.[0],
	...
	setLatestTrackedCommit(dotGitFilePath: string, commit: string) {
		...
		setJsonItem(getGitEventFile(), dotGitFilePath, {
	...
	removeBranchFromTrackingHistory(dotGitFilePath: string) {
		const data = getFileDataAsJson(getGitEventFile());
